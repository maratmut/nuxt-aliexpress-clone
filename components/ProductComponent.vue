    ...
    <NuxtLink :to="`/item/${product.id}`"
      ...
      <div id="ProductDetails">
        <span class="flex items-center justify-start gap-3 px-1 pt-1">
          <span class="text-[#FF6674]">₽{{ priceComputed }}</span>
          <span class="text-gray-500 text-sm text-light line-through">₽{{ oldPriceComputed }}</span>
        </span>
        <span class="px-2 relative -top-1.5 text-[#FF6674] text-xs font-semibold"
          >Доп. скидка 5%</span
        >
        ...
        <div class="flex items-center gap-1 px-1 relative -top-1">
          <span class="bg-[#FD374F] text-white text-[9px] font-semibold px-1.5 rounded-sm"
            >Добро пожаловать в сделку</span
          >
          <span
        ...
        <p class="flex items-center px-1 pt-0.5 text-sx text-[#252525]">
          5,000+ продано
          <Icon name="material-symbols:star-rate" color="#757575" class="ml-1.5" /> 4.7
        </p>
        <p class="px-1 pt-0.5 text-xs text-[#252525]">{{ product.title.substring(0, 69) }}</p>
        <p class="px-1 pb-1">
          <span class="text-[#009A66] text-xs font-semibold">Бесплатная доставка</span>
        </p>
      </div>
    </NuxtLink>
  </div>
